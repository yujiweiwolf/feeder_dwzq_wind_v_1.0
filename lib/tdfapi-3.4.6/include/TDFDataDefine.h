...
{	
    ...
    struct TD_Info_Shares
    {
		...
		char chProfitFlag;                   // 表示“目前是否尚未盈利”, 0: 无数据, 1: 已盈利,  2：未盈利
		char chWeightedVotingRights;         // 表示“是否存在投票权差异”, 0: 无数据, 1: 不存在, 2: 存在差异
		char chExtType;						 // 扩展类型, [对于全国股转系统股票，是分层信息, 0: 无数据, 1: 基础层, 2: 创新层， 3: 精选层];
											 // [对深交所系统, 用8个位分别保存状态, 最低位为1时表示: 上市初期(chExtType&0x1)  ]
		char chTradeType;					 // 交易类型, [对于全国股转系统股票，‘T’表示协议交易方式；‘M’表示做市交易方式；‘B’表示集合竞价+连续竞价交易方式；‘C’表示集合竞价交易方式；‘P’表示发行方式，‘O’表示其他类型]
		char chIsRegistration;				 // 是否注册制, 0: 无数据, 1: 是, 2: 否
		char chIsVIE;						 // 是否具有协议控制架构, 0: 无数据, 1: 是, 2: 否
		char chCrdBuyUnderlying;			 // 是否为融资标的, 0: 无数据, 1: 是, 2: 否
		char chCrdSellUnderlying;			 // 是否为融券标的, 0: 无数据, 1: 是, 2: 否
    }Shares;
    //权证
...
union TD_Exdata
{
	...
	struct TD_Exdata_ETF
	{
		...
		int nETFBuyNumber;       // ETF申购笔数(上交所)			
		int nETFSellNumber;      // ETF赎回笔数(上交所)			
		int64_t iETFBuyAmount;   // 本字段为复用：ETF申购数量(上交所)/盘前量（美股基金）		
		int64_t iETFBuyMoney;    // 本字段为复用：ETF申购金额(上交所)/盘前价格（美股基金）
		int64_t iETFSellAmount;  // 本字段为复用：ETF赎回数量(上交所)/盘后量（美股基金）
		int64_t iETFSellMoney;   // 本字段为复用：ETF赎回金额(上交所)/盘后价格（美股基金）
		int	nTime;				 // 盘前/盘后时间（美股基金）
	}ETF;
...
struct TDMarketData_Packet
{
    ...
    char	chStatusFlag;          //交易状态标志
									...
									//e 有盘后交易的品种的正常交易时间结束(比如科创板15:00时交易结束)
	char    chTradeFlag;           // 交易标志
